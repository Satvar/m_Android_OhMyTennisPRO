...
public class NavigationAdapter extends BaseAdapter {
	...
	public void setMapsCounter(int count){
		mMapsCounter = count;
		mList.get(Constant.MENU_LOGOUT).counter = mMapsCounter;
		notifyDataSetChanged();
	}
	...
	class ViewHolder {
		...
		public RelativeLayout listvisible;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.listvisible = (RelativeLayout) convertView.findViewById(R.id.listvisible);
		...
		if (holder.listvisible != null) {
			if (item.isVisible) {
				holder.listvisible.setVisibility(View.VISIBLE);
			} else {
				holder.listvisible.setVisibility(View.GONE);
			}
		}
		if (holder.icon != null) {
			if (item.icon != 0) {
